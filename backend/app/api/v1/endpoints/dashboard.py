...
async def get_employer_dashboard(
    ...
    # Get basic stats
    total_jobs = (
        db.query(func.count(Job.id)).filter(Job.employer_id == current_user.id).scalar()
    )
    active_jobs = (
        db.query(func.count(Job.id))
        .filter(and_(Job.employer_id == current_user.id, Job.is_active == True))
        .scalar()
    )
    ...
    total_candidates = (
        db.query(func.count(func.distinct(JobApplication.applicant_id)))
        .join(Job)
        .filter(Job.employer_id == current_user.id)
        .scalar()
    )
    ...
    return {
        ...
        "recent_candidates": [
            {
                ...
                "name": candidate.applicant.display_name
                or f"{candidate.applicant.first_name} {candidate.applicant.last_name}",
        ...
        "application_status": [
            {"name": status, "value": count} for status, count in status_distribution
        ],
    }
